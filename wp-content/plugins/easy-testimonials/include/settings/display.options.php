	...
	function output_date_options(){
		...
		<table class="form-table">
			<tr valign="top">
				...
				<td><input type="text" name="easy_t_date_format" id="easy_t_date_format" value="<?php echo get_option('easy_t_date_format', get_option('date_format') ); ?>" />
				<p class="description">Use this to change how the Testimonial Date is formatted.  Our default format is "<?php echo date( get_option('date_format') ); ?>" <code><?php echo get_option('date_format');?></code>. This follows the standard used for <a href="http://php.net/manual/en/function.date.php">PHP date()</a></p>
				<p class="description">
					<strong>Examples:</strong><br>
					<ul>
						<li><?php echo date('F j, Y'); ?>	<code>F j, Y</code></li>
						<li><?php echo date('Y-m-d'); ?>	<code>Y-m-d</code></li>
						<li><?php echo date('m/d/Y'); ?>	<code>m/d/Y</code></li>
						<li><?php echo date('d/m/Y'); ?>	<code>d/m/Y</code></li>
					</ul>
				</p>
				</td>
			</tr>
						...
						<?php $group_label = $this->get_theme_group_label($theme_group); ?>									
							<optgroup label="<?php echo esc_attr($group_label);?>">
								<?php foreach($theme_group as $key => $theme_name): ?>
									<option value="<?php echo $key ?>" <?php if($current_theme == $key): echo 'selected="SELECTED"'; endif; ?>><?php echo esc_html($theme_name); ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php endforeach; ?>
					</select>
