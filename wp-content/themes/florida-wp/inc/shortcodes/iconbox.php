...
function webnus_iconbox( $attributes, $content = null ) {
	...
	{
		$icon_name = substr($icon_name,8,strlen($icon_name));
		echo wpb_js_remove_wpautop(("[icon name='$icon_name' size='$icon_size' color='$icon_color']"));
	}
...
?><?php
		 ...
		 echo !empty($icon_link_url)?"<a class=\"magicmore\" href=\"{$icon_link_url}\">{$icon_link_text}</a>":'';
